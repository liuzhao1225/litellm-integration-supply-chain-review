...
[五个补充项目](../contributor-samples/README.md)增加了 PyRIT、MemoryOS、bot-on-anything、deep-research-web-ui 和 ha-llmvision 的 44 个文件比较、27 份完整文件及 26 项讨论。合计 45 条 RheagalFire PR 中 27 条已有固定比较，18 条仍只有搜索元数据。PyRIT 后续维护者收紧版本范围，MemoryOS 默认安装与描述不符，两类证据同时保留。[历史索引对照](../historical-resolution/README.md)确认两个恶意版本在当前官方及阿里云索引均无候选；数值范围包含不能直接推导实际安装过。
...
- 45 个 RheagalFire PR 中 18 个尚无固定文件比较；即使标题显示 provider，也不能推定安装了 PyPI 包。优先核对会改变安装或执行判断的具体差异。
